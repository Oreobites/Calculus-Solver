...
public class Exponential_function {
	...
	public static String expo_2(String arr) { //+가 1개 이상 있는 계산식
		...
		for(int i=0;i<arr.length();i++) {
			...
			if(arr.charAt(i)==45) {
				if(i==0) {
					if(arr.charAt(i+1)==0x20) {
						k_2[n_1] = i;
						n_1++;
					}
				} else {
					if(arr.charAt(i+1)==0x20||arr.charAt(i-1)==120) {
						k_2[n_1] = i;
						n_1++;
					}
				}
		...
		for(int i=0;i<=n_1;i++) {
			...
			if(a==0) { //*없으면
				...
				if(d) { //지수가 자연로그이면
					for(int j=k_1[i];j<f;j++) {
						if(48<=arr.charAt(j) && arr.charAt(j)<=57) 
							g=true;
					}
					if(g) { //e^nx이면
				...
				for(int j=s;j<k_1[i];j++) {
					if(arr.charAt(j)==42)
						break;
					if(arr.charAt(j)==43||arr.charAt(j)==45)
						continue;
					multiple += arr.charAt(j);
					//*앞에 있는 상수를 multiple에 저장
				}
